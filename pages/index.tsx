...
const Plot = dynamic(() => import("plotly.js-dist-min"), { ssr: false }) as any;
...
export default function Home() {
  ...
  if (!snap) return <main style={{padding:20}}>Loading… (run <code>/api/compute</code> if first time)</main>;
  ...
  return (
    <main style={{padding:20, fontFamily:"system-ui"}}>
      ...
      {/* Gauge (simple text; you can add a proper Plotly gauge if desired) */}
      <div style={{fontSize:24, fontWeight:700, margin:"12px 0"}}>
        Macro Stress Breaches: {snap.breach_count}
      </div>
      ...
      <p style={{marginTop:20, fontSize:12, color:"#666"}}>
        Tip: The site updates after the scheduled compute function writes a new snapshot to KV.
      </p>
    </main>
  );
}
